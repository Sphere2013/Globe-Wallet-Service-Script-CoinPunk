 ...
 *
 * Returns the transaction as a byte array in the standard Globe binary
 * format. This method is byte-perfect, i.e. the resulting byte array can
 * be hashed to get the transaction's standard Globe hash.
 */
 ...
 *
 * Globe uses a different hash for each signed transaction input. This
...
Transaction.prototype.analyze = function (wallet) {
  ...
  if (impact.sign > 0 && impact.value.compareTo(BigInteger.ZERO) > 0) {
    analysis.type = 'recv';
    analysis.addr = new Globe.Address(firstMeRecvHash);
    ...
    //       change address.
    analysis.addr = new Globe.Address(firstRecvHash);
  } else  {
    analysis.type = "other";
  }
...
Transaction.prototype.calcImpact = function (wallet) {
  if (!(wallet instanceof Globe.Wallet)) return BigInteger.ZERO;
