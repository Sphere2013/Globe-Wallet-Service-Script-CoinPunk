...
var Wallet = function (globeNetwork) {
  // Keychain
  //
  ...
  this.addressHashes = [];
  this.globeNetwork = globeNetwork;
  ...
  this.addKey = function (key, pub) {
    ...
    this.addressHashes.push(key.getGlobeAddress(this.globeNetwork).getHashBase64());
  };
  ...
  this.getAllAddresses = function () {
    var addresses = [];
    for (var i = 0; i < keys.length; i++) {
      addresses.push(keys[i].getGlobeAddress(this.globeNetwork));
    }
    return addresses;
  };
  ...
  this.getCurAddress = function () {
    if (keys[this.addressPointer]) {
      return keys[this.addressPointer].getGlobeAddress();
    } else {
      return null;
    }
  };
  ...
  this.getNextAddress = function () {
    ...
    */
    ...
    return keys[this.addressPointer].getGlobeAddress(this.globeNetwork);
  };
...
Wallet.prototype.hasHash = function (hash) {
  if (Globe.Util.isArray(hash)) hash = Crypto.util.bytesToBase64(hash);
