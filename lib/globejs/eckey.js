...
var ECKey = function (input) {
  ...
  if (!input) {
    ...
    // A list of base58 encoded prefixes is at https://en.globe.it/wiki/List_of_address_prefixes.
...
ECKey.prototype.getGlobeAddress = function (address_type) {
  var hash = this.getPubKeyHash();
  var addr = new Address(hash, address_type);
  return addr;
};

ECKey.prototype.getExportedPrivateKey = function (globeNetwork) {
  globeNetwork = globeNetwork || 'prod';
  var hash = this.priv.toByteArrayUnsigned();
  while (hash.length < 32) hash.unshift(0);
  hash.unshift(networkTypes[globeNetwork]);
